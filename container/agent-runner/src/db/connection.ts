...
/**
 * Background heartbeat ticker — keeps the heartbeat fresh while the poll
 * loop is blocked on a long Claude tool call (pre-commit hooks, builds,
 * tests). Without this, the host's absolute-ceiling sweep kills the
 * container after 30 min of subprocess work even though it's making
 * legitimate progress. The ticker runs forever until shutdown; it's
 * cheap (one fs.utimesSync per tick) and idempotent with the
 * poll-loop's own touchHeartbeat call.
 */
export function startHeartbeatTicker(intervalMs = 10_000): void {
  if (_heartbeatTimer) return;
  // Touch immediately so a fresh process has a baseline mtime even if it
  // dies before the first interval fires.
  touchHeartbeat();
  _heartbeatTimer = setInterval(touchHeartbeat, intervalMs);
  // Don't keep the event loop alive just for this timer — the poll loop
  // owns process liveness.
