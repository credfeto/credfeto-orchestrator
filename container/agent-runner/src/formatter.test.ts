...
import { formatMessages, stripInternalTags, isRateLimitText, parseRateLimitReset } from './formatter.js';
...
describe('isRateLimitText', () => {
  it("matches You've hit your limit at the start", () => {
    expect(isRateLimitText("You've hit your limit · resets 2:40pm (Europe/London)")).toBe(true);
  });

  it('matches the spelled-out variant', () => {
    expect(isRateLimitText('You have hit your limit · resets 9:00am (UTC)')).toBe(true);
  });
  ...
  it("doesn't match unrelated messages mentioning a limit", () => {
    expect(isRateLimitText('Reminder: stay under the token limit when you reply.')).toBe(false);
  });
...
describe('parseRateLimitReset', () => {
  it('parses h:MMpm with a named timezone', () => {
    // 2026-05-24 13:00 UTC = 14:00 Europe/London (BST). Reset "2:40pm London" = 13:40 UTC.
    const now = new Date('2026-05-24T13:00:00Z');
    const ms = parseRateLimitReset("You've hit your limit · resets 2:40pm (Europe/London)", now);
    expect(ms).not.toBeNull();
    expect(new Date(ms!).toISOString()).toBe('2026-05-24T13:40:00.000Z');
  });

  it('parses bare hour without minutes', () => {
    // "9am UTC" should resolve to 09:00:00Z.
    const now = new Date('2026-05-24T08:00:00Z');
    const ms = parseRateLimitReset("You've hit your limit · resets 9am (UTC)", now);
    expect(ms).not.toBeNull();
    expect(new Date(ms!).toISOString()).toBe('2026-05-24T09:00:00.000Z');
  });

  it('rolls to tomorrow when the parsed time is already past', () => {
    // "2:40pm London" with now=15:00 UTC (16:00 London) → reset already past today,
    // so the next occurrence is tomorrow at 13:40 UTC.
    const now = new Date('2026-05-24T15:00:00Z');
    const ms = parseRateLimitReset("You've hit your limit · resets 2:40pm (Europe/London)", now);
    expect(ms).not.toBeNull();
    expect(new Date(ms!).toISOString()).toBe('2026-05-25T13:40:00.000Z');
  });

  it('returns null when the time pattern is missing', () => {
    expect(parseRateLimitReset("You've hit your limit, try later", new Date('2026-05-24T13:00:00Z'))).toBeNull();
  });

  it('returns null on a bogus timezone string', () => {
    expect(
      parseRateLimitReset("You've hit your limit · resets 2:40pm (Not/A_Zone)", new Date('2026-05-24T13:00:00Z')),
    ).toBeNull();
  });
});
