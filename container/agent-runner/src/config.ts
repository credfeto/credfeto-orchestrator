...
export interface RunnerConfig {
  ...
  /**
   * If set (non-empty), a bare agent-to-agent "ping" is answered with this
   * exact text directly by the poll loop, with NO provider/LLM call — a
   * connectivity probe should cost zero tokens. Unset = no special handling
   * (the message goes to the agent like anything else).
   */
  pingReply: string;
}
...
export function loadConfig(): RunnerConfig {
  ...
  _config = {
    ...
    pingReply: (raw.pingReply as string) || '',
  };
