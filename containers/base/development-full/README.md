...
- `cfwf` (Credfeto WorkFlow) - one flat command for each recurring multi-step `gh` pattern, so a single `command-allowlist`/`Bash(cfwf *)` entry covers it instead of the agent composing a fresh multi-statement script each time. `cfwf workflow-status --set --repo <owner/repo> (--pr <n> | --issue <n>) --status <name>` adds an issue or PR to the Workflow board, and sets its Workflow Status and then the project's built-in `Status` to match (Todo for Not Started and Planning, In Progress for Approved through Human Review, Done for Complete; skipped with a warning if the project has no such option), trusting `gh project item-edit`'s exit status and never reading the values back (a write whose call succeeded is done: GitHub can lag behind a write by seconds, so a read straight afterwards that disagrees is lag, not a failed write, and is checked again at a later step rather than polled or repeated), with the board, field and option all looked up from `--repo` and `--status` so no project, field or option ids are needed; `cfwf workflow-status --check` takes the same `--repo`/`--pr`/`--issue` and prints its current status on one line as `<Workflow Status> (<built-in Status>)`, for example `Development (In Progress)`; `cfwf closing-issue-labels` prints the labels of the issues a PR closes, without `Blocked`/`On-Hold`. `cfwf issue create --repo <owner/repo> --priority <Security|Urgent|High|Medium|Low> --title <title> --body-file <file|-> [--label <label> ...]` creates an issue and puts it on the Workflow board as `Not Started` in one step, printing only the issue URL: it validates everything (including that a Workflow board is linked) before creating anything, refuses a priority label given as `--label`, and creates any label the repository lacks first (a standard label from `.github/labels.yml` gets its standard colour and description, hard-coded in `cfwf` and kept in step by `test/standard-labels-parity.bats`; any other label gets `gh`'s default colour). Uses native `gh repo`/`gh project`/`gh pr`/`gh issue` subcommands for everything, and every write is a native `gh project` command. Its one board read (`--check`) uses a read-only single-item GraphQL query (the only use of `gh api graphql` in `cfwf`) because `gh project` has no command that reads one item and listing the whole board is capped and can be slow to show a freshly added item (#1491); if that query fails, `cfwf` warns on stderr and falls back to listing the board (up to 10000 items). `cfwf help` lists the options; a bare `cfwf` prints the usage to stderr and exits 2. `install-claude-hooks` also installs it into `/usr/local/bin` on a host, using `sudo` if the plain install fails (`CFWF_BIN_DIR` to override the directory).
