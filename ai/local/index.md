...
* [Claude Hooks](claude-hooks.instructions.md) — `reject-obfuscated-commands`/`enforce-allowed-dirs`/`enforce-git-identity`/`enforce-git-dash-c`/`block-git-worktree`/`block-dotnet-tool-install` guardrail hooks: layered blocking policy, the `allowed-dirs` directory allowlist (and its host-side `allowed-dirs.local` override), safe vs unsafe ways to extend them, and the tracking issue for allowlist requests needing human review. Load when a Bash hook blocks a command, or when working on `containers/base/development-full/claude-hooks/`.
* [Fingerprinting](fingerprinting.instructions.md) — the `FINGERPRINT_SCHEMA_VERSION` bump rule (mandatory whenever a hashed field is added to or removed from `fingerprint_issue_json`/`fingerprint_pr_json`), what each function currently hashes, and why an unbumped version can silently strand an Issue/PR. Load when working on `lib/fingerprints`, or on any code that computes `plan_approved`/board-approval status feeding into it.
* [Development Guides](development-guides.instructions.md) - the repository development guide (`docs/development/README.md`), the per-script guides, `lib.md` and `claude-hooks.md`: read the relevant guide before changing a script, `lib/*` module, hook or test, and update it in the same PR as any behaviour change. Load when changing any script, `lib/*` module, Claude Code hook or `*.bats` test, or adding a script.
