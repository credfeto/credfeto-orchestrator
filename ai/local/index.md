...
* [Claude Hooks](claude-hooks.instructions.md) — `reject-obfuscated-commands`/`enforce-git-identity`/`enforce-git-dash-c`/`block-git-worktree`/`block-dotnet-tool-install` guardrail hooks: layered blocking policy, safe vs unsafe ways to extend them, and the tracking issue for allowlist requests needing human review. Load when a Bash hook blocks a command, or when working on `containers/base/development-full/claude-hooks/`.
