...
- `total` — every agent invocation ever spent on the PR. At `MAX_PR_TOTAL_INVOCATIONS` (computed in lib/globals from the four per-phase round budgets plus a flat headroom; 57 by default) the PR is marked Blocked. A PR stuck at a high total that never merges is churning without converging.
