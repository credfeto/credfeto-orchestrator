...
| Plan-approved Issue marked `Blocked` with "no further progress ... idle-invocation budget ... exhausted", although the agent comments show it deferring because another PR was open in the same repo | The shell never defers an Issue for a PR that is not the orchestrator own: only PRs authored by `_GH_ME` or `PR_CREATOR_LOGIN` occupy its slot, and `find_open_nonblocked_pr_for_repo` (Blocked PRs included) and `find_human_taken_over_pr_for_issue` already stand it off for those. So the session itself deferred, reading the one-active-PR rule ("per user") as covering someone else PR, and the idle counter climbed to the cap. The cause is the agent instruction wording, which is maintained upstream in `cs-template`, not the shell | Read the agent transcript for the deferral and the PR it named, then compare that PR author with `_GH_ME` and `PR_CREATOR_LOGIN`; `oneshot` (`block_issue_for_idle_exhausted_no_progress` call site) |
