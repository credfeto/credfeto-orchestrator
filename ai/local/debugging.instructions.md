...
Since #1298, staleness is largely self-reported rather than something you need to go hunting for: the merge step retries once, clearing any stale `*.lock` file anywhere under `.git` first, if the first attempt fails — the exact failure seen in that incident, a lock left by a previous merge killed mid-write by the step's own `timeout`. A `find`, not a top-level glob, is required here: a merge killed while updating the branch ref leaves its lock nested at `.git/refs/heads/<branch>.lock`, not directly under `.git/`. The deletion is age-gated (locks younger than a minute are left alone) — `REPO_DIR` is the same checkout for every `--owner` variant installed for a given user, so an unconditional delete could remove a lock a *different*, concurrently-running unit's merge is still legitimately holding, not just one abandoned by a killed process. If the checkout is still behind `origin/main` after that, `oneshot` itself detects it (`git_commits_behind` in `lib/git`) and, **only when systemd-invoked** (gated on `ORCHESTRATOR_SELF_UPDATE_MANAGED` — a variable install-timer's generated unit declares explicitly, deliberately not systemd's generic `INVOCATION_ID`, which is set for any unit's main process and would also wrongly catch a manual shell descended from an unrelated systemd session; this deliberately does not apply to `loop`, which has its own looser "warn and keep going" recovery policy for the same underlying condition, #1104, or to a manual/interactive invocation), posts a Discord alert (`docs/discord-notifications.md`) and exits non-zero — so `systemctl status`/`journalctl` also show the unit as `failed`, not a normal successful run. The commands above remain the fallback for diagnosing *why* a merge keeps failing (a genuinely diverged branch, an auth/network failure, or some other stale-lock shape not yet handled) once the alert or a failed unit points you here.
...
| Plan-approved Issue marked `Blocked` with "no further progress ... idle-invocation budget ... exhausted", although the agent comments show it deferring because another PR was open in the same repo | The shell never defers an Issue for a PR that is not the orchestrator's own: only PRs authored by `_GH_ME` or `PR_CREATOR_LOGIN` occupy its slot, and `find_open_nonblocked_pr_for_repo` (Blocked PRs included) and `find_human_taken_over_pr_for_issue` already stand it off for those. So the session itself deferred, reading the one-active-PR rule ("per user") as covering someone else's PR, and the idle counter climbed to the cap. The cause is the agent instruction wording, which is maintained upstream in `cs-template`, not the shell | Read the agent transcript for the deferral and the PR it named, then compare that PR author with `_GH_ME` and `PR_CREATOR_LOGIN`; `oneshot` (`block_issue_for_idle_exhausted_no_progress` call site) |
