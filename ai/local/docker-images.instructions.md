...
- **Overwrite, never append.** Each stage's `RUN` writes the file fresh (`>`, not `>>`), so it ends up containing only that stage's own additions — a downstream image inherits the file via `FROM` but immediately replaces it with its own content. A stage that genuinely adds no new tools (e.g. `development-agent`, which only removes package managers) still overwrites it with an explanatory line rather than leaving the inherited file in place.
