...
4. Run `/code-review --comment` (up to `MAX_CODE_REVIEW_ITERATIONS` rounds; board status **AI Review**); fix each inline finding in its own commit. After each fix, run Changelog (correction) and commit `CHANGELOG.md` separately if the entry changed, then push. Before fixing a round's findings, judge convergence against the PR's history of prior code-review comments: if this round is substantially repeating findings already reported (not doing anything new) AND at least `MIN_REVIEW_CONVERGENCE_ROUNDS` rounds have now run, stop early, post a comment, and advance to security review without the Blocked label instead of continuing to iterate; below that floor, treat even a repeat as one failed fix attempt and keep fixing, mirroring `SIMPLIFY_THRASH_LIMIT`'s role in step 3 above. (Not yet reflected in `agent-roles.instructions.md`, which is maintained upstream in `credfeto/cs-template` and still references the old, now-removed `MAX_REVIEW_ITERATIONS` name for this and the security-review step below instead of the current `MAX_CODE_REVIEW_ITERATIONS`/`MAX_SECURITY_REVIEW_ITERATIONS` split; see `credfeto/cs-template#1036`.)
5. Run `/security-review` (up to `MAX_SECURITY_REVIEW_ITERATIONS` rounds; board status **AI Security Review**); fix each finding in its own commit. After each fix, run Changelog (correction) and commit `CHANGELOG.md` separately if the entry changed, then push. Same self-detected early-exit as step 4 applies, judged against the PR's prior security-review comments.
