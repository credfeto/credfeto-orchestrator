...
4. Run `/code-review --comment` (up to `MAX_CODE_REVIEW_ITERATIONS` rounds; board status **AI Review**); fix each inline finding in its own commit. After each fix, run Changelog (correction) and commit `CHANGELOG.md` separately if the entry changed, then push. Before fixing a round's findings, judge convergence against the PR's history of prior code-review comments: if this round is substantially repeating findings already reported (not doing anything new), stop early, post a comment, and advance to security review without the Blocked label instead of continuing to iterate. (Not yet reflected in `agent-roles.instructions.md`, which is maintained upstream in `credfeto/cs-template` and still references the old, now-removed `MAX_REVIEW_ITERATIONS` name for this and the security-review step below instead of the current `MAX_CODE_REVIEW_ITERATIONS`/`MAX_SECURITY_REVIEW_ITERATIONS` split; see `credfeto/cs-template#1036`.)
