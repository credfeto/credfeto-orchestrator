...
It sources nothing from `lib/`; `die`, `success`, `info` and `is_ai_agent` are local copies. Required tools (`check_required_tools`): `id`, `sudo`, `systemctl`, `git`, `getent`. The generated units also rely on absolute-path tools that are not checked at install time (`find`, `chown`, `mkdir`, `rm`, `chmod`, `timeout`, `pkill`, `ssh-agent`, `gpgconf`, `podman`, `xargs`, and `/bin/sh`).

1. At load time (also when sourced) it computes `REPO_DIR` (checked for newlines only), `CURRENT_USER` (newline and `^[a-zA-Z0-9._-]+$` checks), `SERVICE_NAME` (regex check) and the constant `UNIT_DIR`.
...
`test/install-timer.bats` (26 tests). Setup uses `setup_isolated_env`, `make_stub systemctl`, exported `id`/`getent`/`sudo` functions and `source_install_timer`, with `cleanup_stubs` in teardown. The two stale-lock tests additionally use `setup_local_git_remote` and `advance_remote_main`.
...
- [ ] `shellcheck install-timer test/install-timer.bats` is clean. `ai/local/shell-testing.instructions.md`'s top-level list omits the `.bats` file (it says to run `shellcheck test/*.bats` separately); run both.
...
- Do not add `KillMode=`, `StartLimitIntervalSec=` or `StartLimitBurst=` (tests assert their absence, reasoning in the script header; for `KillMode` see the `ExecStop` comment in the unit body, since the header NOTE covers only `StartLimit*`). Do not add `PrivateTmp` or `NoNewPrivileges=yes` to the main unit. The strict settings belong only to the failure unit.
- All `--owner` units for one user share the same `REPO_DIR` checkout, which is why the lock deletion is age-gated (`-mmin +1`).
- `REPO_DIR` is only checked for newlines and is unquoted in both `ExecStart=` lines (`oneshot` and `notify-unit-failure`), and systemd splits `ExecStart=` command lines on whitespace, so a path containing spaces would make the unit fail. The `ExecStartPre` lines do quote it. This is untested.
- A re-run issues `daemon-reload`, `enable` and `start`, never `restart`. Whether an already-running timer picks up a changed interval without a restart is not covered by the script or tests.
