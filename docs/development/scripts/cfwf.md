...
5. `gh issue create --repo ... --title ... --body-file <path> --label ...` for every label. The last line of its output must look like `https://github.com/<owner>/<repo>/issues/<n>`, otherwise it dies saying the issue may have been created. The owner and repository are not compared with `--repo`, because a renamed or transferred repository answers with its new name and the issue exists by then.
