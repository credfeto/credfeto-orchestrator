...
5. `gh issue create --repo ... --title ... --body-file <path> --label ...` for every label. The last line of its output becomes `CREATED_URL`, which makes the `EXIT` trap report it if the run then fails, and must look like `https://<host>/<owner>/<repo>/issues/<n>`, otherwise it dies saying the issue may have been created. Neither the host nor the owner and repository are compared with anything: `gh` may be reached through a proxy that answers with its own host, and a renamed or transferred repository answers with its new name.
6. `put_item_on_board` (the code `--set` uses) adds the item and sets both statuses, and the URL is printed.
