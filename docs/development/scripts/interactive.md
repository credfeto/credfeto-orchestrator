...
- `SSH_AUTH_SOCK` and `CLAUDE_CODE_AUTO_COMPACT_WINDOW` (default 100000, `lib/globals`) are read from your shell. `GH_HOST` from your shell is used only on first run, to write `.env` (default `github.com`); afterwards the `.env` value wins (`load_env_config` exports it, and `require_container_gh_token` dies unless `.env` supplies a `GH_HOST`/`GH_TOKEN` pair).
...
1. Source `lib/globals`, `lib/core`, `lib/git`, `lib/github`, `lib/github-status`, `lib/fingerprints`, `lib/state`, `lib/prompts`, `lib/workflow-board`, `lib/discord` and `lib/podman`, in that order. Each `source` has a dependency-free `FATAL` fallback, because `die` does not exist until `lib/core` has loaded. Only `lib/state`'s pull-duration helpers (`pull_duration_exceeds_baseline`, `record_pull_duration`, reached through `ensure_agent_container_ready`) are used from those five libraries. Nothing from `lib/github`, `lib/github-status`, `lib/fingerprints` or `lib/workflow-board` is reachable from `main`; they are sourced only to match `oneshot`. (Found by a static call-graph walk, so approximate.)
