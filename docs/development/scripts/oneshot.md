...
- Environment variables that matter (defaults and validation in `lib/globals`):
  ...
  - Identity: `PR_CREATOR_LOGIN` (default `prpixie`), the PR create bot whose PRs count as the orchestrator's own, alongside the account its token belongs to.
...
| Which PR prompt is used | `pr_should_use_dependency_prompt` (lib/github): branch prefix `depends/` or `dependabot/` only, never a label, and never while changes are requested, `.deleteme.now` is present, or the PR is from a fork (`isCrossRepository`). |
...
- `gh pr list --author @me` is avoided (broke in gh 2.93.0); `list_bot_created_open_prs` filters client-side on the author being `_GH_ME` or `PR_CREATOR_LOGIN`.
...
- GitHub behaviour to allow for (durations are taken from the [development guide](../README.md), not measured here):
  ...
  - List endpoints can lag a new PR. `resolve_resumable_issue_branch` (`gh pr list --head`) has no guard beyond failing closed on an error (a failed count is treated as one open PR) and the next tick.
