...
Discovery reads the field named `Status` from the same fields query and records `_WF_BUILTIN_FIELD_ID` and the option ids in the `builtin_*` keys of `_WF_CACHE` and in the disk cache. `load_project_cache` treats an entry that lacks the `builtin_field_id` key as a miss (#1493), so caches written before #1493 are rediscovered once; an entry that has the key with an empty value (a project with no built-in `Status` field) is a hit, so it is not rediscovered on every run. A project that `discover_or_create_workflow_project` has just created is not written to the disk cache until a later run has seen its built-in field, because the create response carries only the `Workflow Status` field; in that creating run the built-in write is skipped with a warning.
