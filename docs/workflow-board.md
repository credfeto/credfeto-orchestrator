...
7. **Coverage** — the last automated gate: recompute overall test coverage on both `main` and
   the branch, and if the branch's is lower, send the board back to Development instead of
   advancing, so more tests get written before the PR reaches a human. Placed after both review
   phases (not right after Development) because those phases can themselves commit production
   code, and an earlier gate would let those commits degrade coverage unchecked.
