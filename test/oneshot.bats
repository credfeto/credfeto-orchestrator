...
@test "invoke_claude passes CLAUDE_CONTEXT_BUDGET_TOKENS as container env var (#1070)" {
    local args_log="${TEST_TMP}/podman_args"
    mkdir -p "${REPO_WORK_DIR}" "${RULES_DIR}"
    cat > "${STUB_BIN}/podman" << STUBEOF
#!/usr/bin/env bash
...
STUBEOF
    ...
    invoke_claude "test prompt" "" "" "# mock CLAUDE.md" 2>/dev/null
    grep -qx 'CLAUDE_CONTEXT_BUDGET_TOKENS=170000' "${args_log}"
}

@test "invoke_claude respects a CLAUDE_CONTEXT_BUDGET_TOKENS override from the environment (#1070)" {
    local args_log="${TEST_TMP}/podman_args"
    mkdir -p "${REPO_WORK_DIR}" "${RULES_DIR}"
    CLAUDE_CONTEXT_BUDGET_TOKENS=90000
    cat > "${STUB_BIN}/podman" << STUBEOF
#!/usr/bin/env bash
[ "\$1" = "pull" ] && exit 0
[ "\$1" = "inspect" ] && exit 1
...
STUBEOF
    ...
    invoke_claude "test prompt" "" "" "# mock CLAUDE.md" 2>/dev/null
    grep -qx 'CLAUDE_CONTEXT_BUDGET_TOKENS=90000' "${args_log}"
}
