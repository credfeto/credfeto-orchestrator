...
@test "invoke_claude sets TMPDIR, TMP, and TEMP to CONTAINER_SCRATCH_PATH (#1351)" {
    local args_log="${TEST_TMP}/podman_args"
    mkdir -p "${REPO_WORK_DIR}" "${RULES_DIR}"
    cat > "${STUB_BIN}/podman" << STUBEOF
#!/usr/bin/env bash
...
STUBEOF
    ...
    invoke_claude "unique-prompt-marker-xyz" "" "" "# mock CLAUDE.md" 2>/dev/null
    grep -qx "TMPDIR=${CONTAINER_SCRATCH_PATH}" "${args_log}"
    grep -qx "TMP=${CONTAINER_SCRATCH_PATH}" "${args_log}"
    grep -qx "TEMP=${CONTAINER_SCRATCH_PATH}" "${args_log}"
}
