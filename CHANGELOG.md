...
- Agent containers now start with podman --replace, so a container name left reserved in podman's storage layer by an unclean shutdown can no longer wedge the orchestrator - previously a host reboot stranded the name and every invocation failed for 19.5 hours across ~3,000 cycles with no alert. Also adds three detection layers that were all missing: an OnFailure= handler unit that alerts to Discord even when oneshot is too broken to report on itself, systemd start rate limiting so a wedged service stops rather than retrying forever, and a return code fix so the existing consecutive-infra-failure escalation is no longer bypassed. ExecStop/KillMode/TimeoutStopSec now remove the container cleanly on shutdown so the condition stops recurring (#1361)
### Changed
