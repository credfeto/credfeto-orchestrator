...
- Register a second per-invocation --add-dir, a fresh throwaway scratch directory bind-mounted read-write and exposed via TMPDIR/TMP/TEMP, so the agent has somewhere writable and registered with Claude Code's own file-write trust boundary to redirect scratch output to instead of an unregistered /tmp path (#1351 - an experiment testing whether that boundary respects --add-dir for output-redirect targets)
### Deprecated
### Removed
### Deployment Changes
<!--
