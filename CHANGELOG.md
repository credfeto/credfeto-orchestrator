...
- Allow the bracket ('[') test idiom on the reject-obfuscated-commands allowlist, so the mandated background-then-poll pattern (e.g. `[ -d /proc/$PID ]`) no longer strands agent sessions
### Changed
