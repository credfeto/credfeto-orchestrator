...
- enforce-ssh-host-and-key now also blocks a flag (or non-literal word) immediately after the ssh target - real ssh does not stop parsing its own options at the destination, so a re-injected -o/-J/-F there previously bypassed both the host restriction and the key precondition entirely (local command execution via ProxyCommand, or connection redirection via ProxyJump), confirmed against real ssh's own argv parsing (#1315)
### Added
